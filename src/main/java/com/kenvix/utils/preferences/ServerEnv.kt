...
object ServerEnv : ManagedEnvFile() {
    @Description("站点名称")
    val AppName by envOf("Student-Manager")

    @Description("是否启用调试模式，生成环境务必为 false")
    val DebugMode: Boolean by envOf(kotlin.run {
        System.getProperties().containsKey("forceDebugMode") ||
                System.getProperty("isReleaseBuild")?.run {
                    this.runCatching { toBoolean() }.getOrNull()
    ...
    val DatabaseName by envOf("student_manager")
    @Description("MySQL 数据库用户")
    val DatabaseUser by envOf("root")
    @Description("MySQL 数据库密码")
    val DatabasePassword by envOf( "123456")
    ...
    val MongoDatabaseName by envOf("student_manager")
